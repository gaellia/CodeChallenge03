package com.launchpad.codechallenge03.ui.theme

import androidx.compose.ui.graphics.Color

val Purple200 = Color(0xFFBB86FC)
val Purple500 = Color(0xFF6200EE)
val Purple700 = Color(0xFF3700B3)
val Teal200 = Color(0xFF03DAC5)

val DarkBlue = Color(0xFF2589BD)
val LightBlue = Color(0xFF66B6E1)
val DarkGreen = Color(0xFF38686A)
val LightGreen = Color(0xFFA3B4A2)
val OffWhite = Color(0xFFE6E8E6)
val TransparentBlack = Color(0x80000000)
val RichBlack = Color(0xFF091A20)
val LightBrown = Color(0xFF9E8576)
val DarkBrown = Color(0xFF6B584C)
val MediumGreen = Color(0xFF789277)

val DarkestBlue = Color(0xFF090C9B)
val LightestBlue = Color(0xFFAADFEE)